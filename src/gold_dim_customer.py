...
customers = spark.read.format("delta").load(silver_path)
...
dim_customer = (
    ...
    .select(
        "customer_key", "prefix", "first_name", "last_name", "full_name",
        "birth_date", "marital_status", "gender", "email_address",
...
distinct_customer_keys = (
    result.select("customer_key").distinct().count()
)
...
print(f"Duplicate customer keys: {total_customers - distinct_customer_keys}")

null_customer_keys = (
    result.filter(col("customer_key").isNull()).count()
)
print(f"Null customer keys     : {null_customer_keys}")

null_full_names = (
    result.filter(col("full_name").isNull()).count()
)
print(f"Null full names        : {null_full_names}")
